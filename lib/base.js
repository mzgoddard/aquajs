...
Emitter.prototype = {
  on: function(name, f, ctx) {
    ...
    if (this._events[name].indexOf(f) == -1)
      this._events[name].push(f, ctx);
  },
  off: function(name, f, ctx) {
    var index = -1;
    if (!this._events) {
      this._events = {};
    }
    if (this._events[name]) {
      while ((index = this._events[name].indexOf(f,index+1)) !== -1) {
        if (!ctx || this._events[name][index + 1] === ctx) {
          this._events[name].splice(index,2);
          index -= 2;
        }
      }
    }
  ...
  emit: function(name) {
    ...
    if (this._events[name]) {
      ...
      for ( i = 0; i < events.length; i += 2 ) {
        events[i].apply(events[i+1] || this, args);
      }
    }
  }
};
